...
def upload_audio():
    if request.method == 'POST':
        ...
        if transcript.strip():
            for section, prompt in PROMPTS.items():
                ...
                analysis[section] = result if result else {"error": f"Failed to parse {section}"}
        ...
        # Overall score
        context_score = (agent_sentiment_percent + employee_sentiment_percent) / 2 / 10 if agent_sentiment_percent and employee_sentiment_percent else 0
        tone_score = calm_score or 0
        sop_score = 0
        if analysis.get('sop_adherence'):
            sop = analysis['sop_adherence']
            sop_steps = [sop.get('identity_verification'), sop.get('security_questions'),
                         sop.get('two_factor_enabled'), sop.get('account_activity_review'),
                         sop.get('security_best_practices')]
            sop_score = (sum(1 for s in sop_steps if s) / 5)
        agent_score = (agent_sentiment_percent or 0) / 100
        resolved_score = 1 if (analysis.get('call_summary') and str(analysis['call_summary'].get('resolved')).lower() == 'yes') else 0

        overall_score = (0.4 * context_score + 0.3 * tone_score +
                         1.0 * sop_score + 1.0 * agent_score + 1.0 * resolved_score)
        ...
        # Metadata
        department = analysis.get('call_summary', {}).get('department')
        topic = analysis.get('call_summary', {}).get('topic')

        def extract_agent_name(transcript):
            try:
                for line in transcript.splitlines():
                    if 'Agent' in line:
                        name_part = line.split('Agent')[1].strip('():- ')
        ...
        dash_chars = r"[-–—‒−]"
        timestamps = [re.match(r"(\d{2}:\d{2}:\d{2})\s*" + dash_chars, line).group(1)
                      for line in transcript.splitlines() if re.match(r"(\d{2}:\d{2}:\d{2})\s*" + dash_chars, line)]
        ...
        match = re.search(r"(\d{6})", audio_file_agent.filename) or re.search(r"(\d{6})", audio_file_employee.filename)
        call_id_val = match.group(1) if match else "N/A"
        ...
        if analysis.get('sop_adherence'):
            sop_steps_followed = sum(1 for s in analysis['sop_adherence'].values() if s)

        # Insert into MySQL
        try:
            insert_sql = """
            INSERT INTO calls (
                agent_audio, employee_audio, transcript, analysis, 
                segment_times_agent, segment_zcrs_agent, segment_times_employee, segment_zcrs_employee,
                avg_speech_rate, calm_score, vad_times, valence_list, arousal_list, dominance_list,
                agent_sentiment_percent, employee_sentiment_percent, sentiment_flow, overall_score, duration,
                department, topic, agent_name, call_id, user_name, user_email, created_at, call_start, call_end,
                voice_elevation_freq, sop_steps_followed
            ) VALUES (%s, %s, %s, %s,
                      %s, %s, %s, %s,
                      %s, %s, %s, %s, %s, %s,
                      %s, %s, %s, %s, %s,
            ...
            cursor.execute(insert_sql, (
                os.path.basename(audio_path_agent),
                os.path.basename(audio_path_employee),
                transcript,
                json.dumps(analysis),
                json.dumps(segment_times_agent), json.dumps(segment_zcrs_agent),
                json.dumps(segment_times_employee), json.dumps(segment_zcrs_employee),
                avg_speech_rate_agent, calm_score,
                json.dumps(vad_times), json.dumps(valence_list),
                json.dumps(arousal_list), json.dumps(dominance_list),
                agent_sentiment_percent, employee_sentiment_percent,
                json.dumps(sentiment_flow), overall_score, duration,
                department, topic, agent_name, call_id_val,
                session.get("user", {}).get("name", "Unknown"),
                session.get("user", {}).get("preferred_username", ""),
                datetime.utcnow(), call_start, call_end,
                voice_elevation_freq, sop_steps_followed
            ))
            conn.commit()
    ...
    for call in rows:
        ...
        call_summary = {}
        try:
            analysis = json.loads(call.get("analysis") or "{}")
        ...
        def get_sentiment_label_and_icon(percent):
            ...
            try:
                percent = float(percent)  # safely convert to number
        ...
        sentiment_label, sentiment_icon = get_sentiment_label_and_icon(call.get("agent_sentiment_percent"))

        match = re.search(r"(\d{6})", call.get("agent_audio", "")) or re.search(r"(\d{6})", call.get("employee_audio", ""))
        call_id_val = match.group(1) if match else "N/A"

        call_list.append({
            "_id": call["id"],   # MySQL auto ID
            "call_id": call_id_val,
            "timestamp": call.get("created_at"),
            "duration": call.get("duration", "N/A"),
            "agent_name": agent_name,
            "department": call.get("department") or call_summary.get("department", "N/A"),
            "topic": call.get("topic") or call_summary.get("topic", "N/A"),
...
@login_required
def dashboard():
    cursor = conn.cursor(dictionary=True)  # dictionary cursor = no more tuple unpacking
    cursor.execute("SELECT * FROM calls ORDER BY created_at DESC")
    rows = cursor.fetchall()
    ...
    def get_sentiment_label_and_icon(percent):
        ...
        # if accidentally stored as JSON string (sentiment_flow)
        if isinstance(percent, str) and percent.strip().startswith("["):
            try:
                flow = json.loads(percent)
                scores = [item.get("score") for item in flow if "score" in item]
                if scores:
                    percent = sum(scores) / len(scores) * 100  # normalize to %
                else:
                    return ("N/A", "")
            except Exception:
                return ("N/A", "")
    ...
    for row in rows:
        ...
        sentiment_label, sentiment_icon = get_sentiment_label_and_icon(row.get("agent_sentiment_percent"))

        # Ensure call_id
        call_id = row.get("call_id")
        if not call_id:
            match = re.search(r"(\d{6})", row.get("agent_audio") or "")
            if not match:
                match = re.search(r"(\d{6})", row.get("employee_audio") or "")
        ...
        call_list.append({
            ...
            "duration": row["duration"] if row["duration"] else "N/A",
            "agent_name": row["agent_name"] or extract_agent_name(row["transcript"]),
            "department": row["department"] or call_summary.get("department", "N/A"),
            "topic": row["topic"] or call_summary.get("topic", "N/A"),
            "resolved": call_summary.get("resolved", "N/A"),
            "sentiment": sentiment_label,
            "sentiment_icon": sentiment_icon,
            "overall_score": row["overall_score"] or "N/A",
            "valence_list": json.loads(row["valence_list"]) if row["valence_list"] else [],
            "arousal_list": json.loads(row["arousal_list"]) if row["arousal_list"] else [],
            "agent_speech_rate": row["avg_speech_rate"] or 0,
            "voice_elevation_freq": row["calm_score"] or "N/A",  # calm_score used as proxy
            "sop_steps_followed": sop_steps_followed,
        })
...
@login_required
def sync_cloud():
    folder_id = '1F5JG1JCG94Pkuj5CPsevbk1x_BqWg5vJ'
    download_dir = os.path.join('Cloud_sync', 'downloaded_transcripts')
    os.makedirs(download_dir, exist_ok=True)
    service_account_json = os.path.join('Cloud_sync', 'credentials.json')
    ...
    for call in results:
        try:
            ...
            cursor.execute("SELECT id FROM calls WHERE call_id = %s", (call_id_val,))
            if cursor.fetchone():
                print(f"[CLOUD SYNC] Skipping duplicate call_id: {call_id_val}")
                continue
            ...
            if transcript.strip():
                for section, prompt in PROMPTS.items():
                    result = generate_section(prompt, transcript)
                    analysis[section] = result if result else {"error": f"Failed to parse {section} output"}
            ...
            context_score = (agent_sentiment_percent + employee_sentiment_percent) / 2 / 10 if agent_sentiment_percent and employee_sentiment_percent else 0
            tone_score = calm_score if calm_score else 0
            ...
            agent_score = agent_sentiment_percent / 100 if agent_sentiment_percent else 0
            resolved_score = 1 if analysis.get('call_summary') and str(analysis['call_summary'].get('resolved', '')).lower() == 'yes' else 0

            overall_score = (0.4 * context_score +
                             0.3 * tone_score +
                             1.0 * sop_score +
                             1.0 * agent_score +
                             1.0 * resolved_score)
            ...
            timestamps = [re.match(r"(\d{2}:\d{2}:\d{2})\s*" + dash_chars, line).group(1)
                          for line in transcript.splitlines()
                          if re.match(r"(\d{2}:\d{2}:\d{2})\s*" + dash_chars, line)]
            call_start = timestamps[0] if timestamps else datetime.utcnow().strftime('%H:%M:%S')
            call_end = timestamps[-1] if len(timestamps) > 1 else None
            ...
            cursor.execute(insert_query, (
                os.path.basename(agent_audio_path),
                os.path.basename(emp_audio_path),
                transcript,
                json.dumps(analysis),
                json.dumps(segment_times_agent),
                json.dumps(segment_zcrs_agent),
                json.dumps(segment_times_employee),
                json.dumps(segment_zcrs_employee),
                float(avg_speech_rate_agent) if avg_speech_rate_agent else None,  # ✅ force float
                float(calm_score) if calm_score else None,  # ✅
                json.dumps(vad_times),
                json.dumps(valence_list),
                json.dumps(arousal_list),
                json.dumps(dominance_list),
                float(agent_sentiment_percent) if agent_sentiment_percent else None,  # ✅
                float(employee_sentiment_percent) if employee_sentiment_percent else None,  # ✅
                json.dumps(sentiment_flow),
...
def view_call(call_id):
    cursor = conn.cursor(dictionary=True)  # ✅ fetch as dicts instead of tuples
    ...
    return render_template(
        "new.html",
        transcript=call.get("transcript"),
        analysis=safe_json_loads(call.get("analysis"), {}),
        audio_file_agent=call.get("agent_audio"),
        audio_file_employee=call.get("employee_audio"),
        segment_times_agent=safe_json_loads(call.get("segment_times_agent"), []),
        segment_zcrs_agent=safe_json_loads(call.get("segment_zcrs_agent"), []),
        segment_times_employee=safe_json_loads(call.get("segment_times_employee"), []),
        segment_zcrs_employee=safe_json_loads(call.get("segment_zcrs_employee"), []),
        avg_speech_rate=call.get("avg_speech_rate"),
        calm_score=call.get("calm_score"),
        vad_times=safe_json_loads(call.get("vad_times"), []),
        valence_list=safe_json_loads(call.get("valence_list"), []),
        arousal_list=safe_json_loads(call.get("arousal_list"), []),
        dominance_list=safe_json_loads(call.get("dominance_list"), []),
        agent_sentiment_percent=call.get("agent_sentiment_percent"),
        employee_sentiment_percent=call.get("employee_sentiment_percent"),
        sentiment_flow=safe_json_loads(call.get("sentiment_flow"), []),
